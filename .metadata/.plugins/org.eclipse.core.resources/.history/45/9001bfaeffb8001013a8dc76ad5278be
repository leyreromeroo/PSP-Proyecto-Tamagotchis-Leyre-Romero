...
public class Tamagotchis implements Runnable {
    ...
    // Mmatar solo si está ocioso/sucio
    public boolean morirPorCuidador() {
        if (estadoActual == Estado.OCIOSO || estadoActual == Estado.SUCIO) {
            ...
            System.out.println(nombre + " está ocupado (" + estadoActual + "), no se puede matar ahora.");
            return false;
        }
    }
    ...
    public void run() {
        ...
        while (enEjecucion.get()) {
            ...
            // 2. --- LÓGICA DE MUERTE POR SUCIEDAD ---
            if (this.suciedad >= 10) {
                this.estadoActual = Estado.MUERTO;
            ...
            // 3. --- LÓGICA DE INCREMENTO DE SUCIEDAD (Cada 20 segundos) ---
            ...
            try {
                // Pausa más larga si está ocioso, más corta si está ocupado revisando estado.
                if (estadoActual == Estado.OCIOSO || estadoActual == Estado.SUCIO) {
                     Thread.sleep(2000); 
                } else {
                     Thread.sleep(500); 
    ...
    public void comer() {
        // Solo puede comer si está OCIOSO o SUCIO
        if (estadoActual != Estado.OCIOSO && estadoActual != Estado.SUCIO) {
            System.out.println(nombre + " está ocupado (" + estadoActual + "), no puede comer ahora.");
            return;
        }
        ...
        System.out.println(nombre + " empieza a comer. Tardará: " + tiempoComida + "ms. [ESTADO: " + estadoActual + "]");
    ...
    public void jugar() {
        if (estadoActual != Estado.OCIOSO && estadoActual != Estado.SUCIO) {
            System.out.println(nombre + " está ocupado (" + estadoActual + "), no puede jugar ahora.");
            return;
        }
        ...
        System.out.println( nombre + " dice: ¡Vamos a jugar! [ESTADO: " + estadoActual + "]");
        ...
        try {
            while (respuesta != suma) {
                ...
                System.out.print("\n [" + nombre + "] ¿Cuánto es " + num1 + " + " + num2 + "? Respuesta: ");
                ...
                if (sc.hasNextInt()) {
                    ...
                    if (respuesta == suma) {
                        System.out.println(nombre + " dice: ¡Correcto! Ganaste. 🎉");
                        break;
                    } else {
                        System.out.println(nombre + " dice: ¡Incorrecto! Vuelve a intentarlo.");
                    }
                } else {
    ...
    public void limpiar() {
        ...
        final long TIEMPO_BAÑO_MS = 5000; 

        System.out.println(nombre + " comienza a limpiarse. Durará 5 segundos. [ESTADO: " + estadoActual + "]");
